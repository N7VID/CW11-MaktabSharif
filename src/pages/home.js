...
<div id="cards" class="grid grid-cols-4 gap-12 mx-auto mt-20 mb-12 max-w-6xl">
  ${countries
    .map((item) => {
      return `
    <div
    class="bg-[#D9D9D9] rounded-2xl overflow-hidden flex justify-center flex-col items-center"
  >
    <div ><img src="${item.flag.large}" class=" object-cover w-52" /></div>
    <div class="font-extrabold text-lg py-4 text-center">${item.name}</div>
    <div
      class="mb-4 bg-[#1400FF] rounded-2xl text-white px-8 py-2 cursor-pointer hover:bg-[#040c45] transition"
    >
